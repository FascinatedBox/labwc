...
#define BOX_FOREIGN_TOPLEVEL_MANAGEMENT_V1_VERSION 10

static const struct zbox_foreign_toplevel_handle_v1_interface toplevel_handle_impl;

static struct box_foreign_toplevel_handle_v1 *toplevel_handle_from_resource(
        struct wl_resource *resource) {
    assert(wl_resource_instance_of(resource,
            &zbox_foreign_toplevel_handle_v1_interface,
...
static void toplevel_handle_send_maximized_event(struct wl_resource *resource,
    ...
    struct box_foreign_toplevel_handle_v1_maximized_event event = {
        .toplevel = toplevel,
        .maximized = state,
    };
...
static void toplevel_send_minimized_event(struct wl_resource *resource,
    ...
    struct box_foreign_toplevel_handle_v1_minimized_event event = {
        .toplevel = toplevel,
        .minimized = state,
    };
...
static void toplevel_send_fullscreen_event(struct wl_resource *resource,
    ...
    struct box_foreign_toplevel_handle_v1_fullscreen_event event = {
        .toplevel = toplevel,
        .fullscreen = state,
        .output = output,
    };
...
static void foreign_toplevel_handle_activate(struct wl_client *client,
    ...
    struct box_foreign_toplevel_handle_v1_activated_event event = {
        .toplevel = toplevel,
...
static void foreign_toplevel_handle_move(struct wl_client *client,
    ...
    if (x < 0 || y < 0) {
        wl_resource_post_error(resource,
            ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_ERROR_INVALID_MOVE,
            "Foreign toplevel move cannot have negative coordinates (have %d, %d).",
            x, y);
        return;
    }

    if (toplevel->state & (ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED |
                           ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_FULLSCREEN |
                           ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MINIMIZED)) {
        // It can't be moved.
        return;
    }

    struct box_foreign_toplevel_handle_v1_move_event event = {
        .toplevel = toplevel,
        .x = x,
        .y = y,
    };
...
static void foreign_toplevel_handle_set_rectangle(struct wl_client *client,
    ...
    if (width < 0 || height < 0) {
        wl_resource_post_error(resource,
            ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_ERROR_INVALID_RECTANGLE,
            "invalid rectangle passed to set_rectangle: width/height < 0");
        return;
    }

    struct box_foreign_toplevel_handle_v1_set_rectangle_event event = {
        .toplevel = toplevel,
...
static const struct zbox_foreign_toplevel_handle_v1_interface toplevel_handle_impl = {
    ...
    .move = foreign_toplevel_handle_move,
...
static void toplevel_idle_send_done(void *data) {
    struct box_foreign_toplevel_handle_v1 *toplevel = data;
    struct wl_resource *resource;
    wl_resource_for_each(resource, &toplevel->resources) {
        zbox_foreign_toplevel_handle_v1_send_done(resource);
    }
...
void box_foreign_toplevel_handle_v1_set_title(
    ...
    struct wl_resource *resource;
    wl_resource_for_each(resource, &toplevel->resources) {
        zbox_foreign_toplevel_handle_v1_send_title(resource, title);
    }
...
void box_foreign_toplevel_handle_v1_set_app_id(
    ...
    struct wl_resource *resource;
    wl_resource_for_each(resource, &toplevel->resources) {
        zbox_foreign_toplevel_handle_v1_send_app_id(resource, app_id);
    }
...
static void send_output_to_resource(struct wl_resource *resource,
    ...
    wl_resource_for_each(output_resource, &output->resources) {
        if (wl_resource_get_client(output_resource) == client) {
            if (enter) {
                zbox_foreign_toplevel_handle_v1_send_output_enter(resource,
                    output_resource);
            } else {
                zbox_foreign_toplevel_handle_v1_send_output_leave(resource,
                    output_resource);
            }
        }
    }
...
static void toplevel_send_output(struct box_foreign_toplevel_handle_v1 *toplevel,
        struct wlr_output *output, bool enter) {
    struct wl_resource *resource;
    wl_resource_for_each(resource, &toplevel->resources) {
...
static void toplevel_handle_output_bind(struct wl_listener *listener,
        void *data) {
    struct box_foreign_toplevel_handle_v1_output *toplevel_output =
...
static void toplevel_handle_output_destroy(struct wl_listener *listener,
    ...
    box_foreign_toplevel_handle_v1_output_leave(toplevel_output->toplevel,
...
void box_foreign_toplevel_handle_v1_output_leave(
        struct box_foreign_toplevel_handle_v1 *toplevel,
        struct wlr_output *output) {
    struct box_foreign_toplevel_handle_v1_output *toplevel_output_iterator;
    struct box_foreign_toplevel_handle_v1_output *toplevel_output = NULL;
...
static bool fill_array_from_toplevel_state(struct wl_array *array,
        uint32_t state) {
    if (state & ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED) {
        uint32_t *index = wl_array_add(array, sizeof(uint32_t));
        if (index == NULL) {
            return false;
        }
        *index = ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED;
    }
    if (state & ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MINIMIZED) {
        uint32_t *index = wl_array_add(array, sizeof(uint32_t));
        if (index == NULL) {
            return false;
        }
        *index = ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MINIMIZED;
    }
    if (state & ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_ACTIVATED) {
        uint32_t *index = wl_array_add(array, sizeof(uint32_t));
        if (index == NULL) {
            return false;
        }
        *index = ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_ACTIVATED;
    }
    if (state & ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_FULLSCREEN) {
        uint32_t *index = wl_array_add(array, sizeof(uint32_t));
        if (index == NULL) {
            return false;
        }
        *index = ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_FULLSCREEN;
    }

    return true;
}

static void toplevel_send_state(struct box_foreign_toplevel_handle_v1 *toplevel) {
    ...
    struct wl_resource *resource;
    wl_resource_for_each(resource, &toplevel->resources) {
        zbox_foreign_toplevel_handle_v1_send_state(resource, &states);
    }
...
void box_foreign_toplevel_handle_v1_set_maximized(
    ...
    if (maximized == !!(toplevel->state &
            ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED)) {
        return;
    }
    if (maximized) {
        toplevel->state |= ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED;
    } else {
        toplevel->state &= ~ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED;
    }
...
void box_foreign_toplevel_handle_v1_set_minimized(
    ...
    if (minimized) {
        toplevel->state |= ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MINIMIZED;
    } else {
        toplevel->state &= ~ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MINIMIZED;
    }
    toplevel_send_state(toplevel);
...
void box_foreign_toplevel_handle_v1_set_activated(
    ...
    if (activated == !!(toplevel->state &
            ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_ACTIVATED)) {
        return;
    }
    if (activated) {
        toplevel->state |= ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_ACTIVATED;
    } else {
        toplevel->state &= ~ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_ACTIVATED;
    }
    toplevel_send_state(toplevel);
...
void box_foreign_toplevel_handle_v1_set_fullscreen(
    ...
    if (fullscreen) {
        toplevel->state |= ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_FULLSCREEN;
    } else {
        toplevel->state &= ~ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_FULLSCREEN;
    }
    toplevel_send_state(toplevel);
...
static void toplevel_resource_send_parent(
    ...
    if (wl_resource_get_version(toplevel_resource) <
            ZBOX_FOREIGN_TOPLEVEL_HANDLE_V1_PARENT_SINCE_VERSION) {
        return;
    }
    ...
    zbox_foreign_toplevel_handle_v1_send_parent(toplevel_resource,
        parent_resource);
}
...
void box_foreign_toplevel_handle_v1_destroy(
    ...
    wl_resource_for_each_safe(resource, tmp, &toplevel->resources) {
        zbox_foreign_toplevel_handle_v1_send_closed(resource);
        wl_resource_set_user_data(resource, NULL);
        wl_list_remove(wl_resource_get_link(resource));
        wl_list_init(wl_resource_get_link(resource));
    }

    struct box_foreign_toplevel_handle_v1_output *toplevel_output, *tmp2;
    ...
    struct box_foreign_toplevel_handle_v1 *tl, *tmp3;
    wl_list_for_each_safe(tl, tmp3, &toplevel->manager->toplevels, link) {
        if (tl->parent == toplevel) {
            ...
            box_foreign_toplevel_handle_v1_set_parent(tl, NULL);
        }
    }
...
static struct wl_resource *create_toplevel_resource_for_resource(
    ...
    zbox_foreign_toplevel_manager_v1_send_toplevel(manager_resource, resource);
    return resource;
}

struct box_foreign_toplevel_handle_v1 *
box_foreign_toplevel_handle_v1_create(
        struct box_foreign_toplevel_manager_v1 *manager) {
    struct box_foreign_toplevel_handle_v1 *toplevel = calloc(1, sizeof(*toplevel));
    if (!toplevel) {
        return NULL;
    }
...
static const struct zbox_foreign_toplevel_manager_v1_interface
    foreign_toplevel_manager_impl;

static void foreign_toplevel_manager_handle_stop(struct wl_client *client,
        struct wl_resource *resource) {
    assert(wl_resource_instance_of(resource,
        &zbox_foreign_toplevel_manager_v1_interface,
        &foreign_toplevel_manager_impl));

    zbox_foreign_toplevel_manager_v1_send_finished(resource);
    wl_resource_destroy(resource);
}
...
static void toplevel_send_details_to_toplevel_resource(
        ...
        struct wl_resource *resource) {
    if (toplevel->title) {
        zbox_foreign_toplevel_handle_v1_send_title(resource, toplevel->title);
    }
    if (toplevel->app_id) {
        zbox_foreign_toplevel_handle_v1_send_app_id(resource, toplevel->app_id);
    }

    struct box_foreign_toplevel_handle_v1_output *output;
...
static void foreign_toplevel_manager_bind(struct wl_client *client, void *data,
    ...
    struct box_foreign_toplevel_manager_v1 *manager = data;
    struct wl_resource *resource = wl_resource_create(client,
            &zbox_foreign_toplevel_manager_v1_interface, version, id);
    ...
    struct box_foreign_toplevel_handle_v1 *toplevel, *tmp;
...
struct box_foreign_toplevel_manager_v1 *box_foreign_toplevel_manager_v1_create(
    ...
    struct box_foreign_toplevel_manager_v1 *manager = calloc(1, sizeof(*manager));
    if (!manager) {
        return NULL;
    }
    ...
    manager->global = wl_global_create(display,
            &zbox_foreign_toplevel_manager_v1_interface,
            BOX_FOREIGN_TOPLEVEL_MANAGEMENT_V1_VERSION, manager,
